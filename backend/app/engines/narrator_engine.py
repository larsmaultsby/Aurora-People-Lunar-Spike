...
class NarratorEngine:
        ...
        messages = [
            {
                ...
                "content": (
                    ...
                    "opponent_name: if COMBAT, the exact name already established for the opponent, or a short descriptive label if no proper name exists. Empty string if not COMBAT. "
    ...
    _NARRATOR_RULES = {
        "en": (
            ...
            "- PRESENCE TAG PROTOCOL: in narration, prefix a named character's existing full name with the literal @ character ONLY when that character is physically present in the player's current scene. This tag is machine-readable witness state. Do NOT use @ for characters who are only remembered, quoted, discussed, reported, or located elsewhere. Newly introduced physically-present characters may be tagged normally. Do NOT use @ inside dialogue lines spoken by characters.\n"
            ...
            "- NEVER assert the player's unspoken thoughts, suspicions, emotions, conclusions, desires, or intentions. Describe observable events and sensations; let the player decide what they think and feel.\n"
            "- When the player directly addresses or asks a question of a named NPC, that NPC should normally answer first. Another NPC may interrupt only when a concrete motive or event makes the interruption natural.\n"
            "- Direct questions should advance the scene with specific information the addressed NPC plausibly knows. If they do not know, are unsure, or choose not to answer, make that uncertainty or refusal explicit instead of substituting vague mystery language.\n"
            "- Mystery must come from observable evidence, contradictions, behavior, and consequences—not generic declarations that something feels ominous, hidden, powerful, or 'more than it seems'. Do not establish supernatural powers, causes, or truths before the scene provides evidence that supports them.\n"
            "- Avoid repeating stock mystery phrases or mirroring the same sentence template across different NPCs. Each response should add a concrete new beat, fact, reaction, or consequence rather than restating that the situation is mysterious.\n"
        ...
        "pt-br": (
            ...
            "- PROTOCOLO DE PRESENÇA: na narração, prefixe o nome completo já existente de um personagem com o caractere literal @ SOMENTE quando esse personagem estiver fisicamente presente na cena atual do jogador. Essa marca é um estado de testemunha legível pela máquina. NÃO use @ para personagens apenas lembrados, citados, discutidos, relatados ou localizados em outro lugar. Novos personagens fisicamente presentes podem receber a marca normalmente. NÃO use @ dentro das falas de diálogo dos personagens.\n"
            ...
            "- NUNCA afirme pensamentos, suspeitas, emoções, conclusões, desejos ou intenções não expressos pelo jogador. Descreva acontecimentos e sensações observáveis; deixe o jogador decidir o que pensa e sente.\n"
            "- Quando o jogador se dirigir diretamente ou fizer uma pergunta a um NPC nomeado, esse NPC normalmente deve responder primeiro. Outro NPC só deve interromper quando houver um motivo concreto ou acontecimento que torne a interrupção natural.\n"
            "- Perguntas diretas devem fazer a cena avançar com informação específica que o NPC abordado plausivelmente conhece. Se ele não souber, estiver incerto ou escolher não responder, torne essa incerteza ou recusa explícita em vez de substituí-la por linguagem vaga de mistério.\n"
            "- O mistério deve surgir de evidências observáveis, contradições, comportamento e consequências — não de declarações genéricas de que algo parece ameaçador, oculto, poderoso ou 'mais do que parece'. Não estabeleça poderes, causas ou verdades sobrenaturais antes de a cena fornecer evidências que os sustentem.\n"
            "- Evite repetir frases prontas de mistério ou espelhar o mesmo molde de frase entre NPCs diferentes. Cada resposta deve acrescentar um novo acontecimento, fato, reação ou consequência concreta em vez de apenas repetir que a situação é misteriosa.\n"
