...
@pytest.mark.asyncio
async def test_openai_proxy_stream_yields_incremental_chunks_and_preserves_proxy_config():
    config = LLMConfig(
        primary_provider=LLMProvider.OPENAI,
        primary_model="local-roleplay",
        temperature=0.85,
        max_tokens=2000,
    )
    router = LLMRouter(config)
    ...
    class FakeStream:
        def __init__(self):
            self._chunks = ["First ", "token stream"]
    ...
    assert chunks == ["First ", "token stream"]
    call_kwargs = mock_acompletion.call_args.kwargs
    ...
    log = get_call_log()
    assert len(log) == 1
    assert log[0]["caller"].endswith("(stream)")
